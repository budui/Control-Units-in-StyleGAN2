...
def convert(ada_path, network_pkl, output_file):
    """
    convert weights from stylegan2-ada-pytorch
    :param ada_path: https://github.com/dvschultz/stylegan2-ada-pytorch local path
    :param network_pkl: path to original network weights
    :param output_file: path to converted network weights
    :return:
    """
    sys.path.append(ada_path)

    import legacy  # noqa
    import dnnlib  # noqa

    with dnnlib.util.open_url(network_pkl) as f:
        ckp = legacy.load_network_pkl(f)
        G_nvidia = ckp["G_ema"]
    ...
    state_dict = {"g_ema": state_ros, "latent_avg": latent_avg, "d": ckp["D"].state_dict()}
